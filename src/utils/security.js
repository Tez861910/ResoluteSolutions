...
class SecurityManager {
  ...
  detectDevTools() {
    ...
    setInterval(() => {
      if (window.outerHeight - window.innerHeight > threshold2 || window.outerWidth - window.innerWidth > threshold2) {
        if (!devtools.open) {
          devtools.open = true;
          devtools.orientation = window.outerHeight - window.innerHeight > threshold2 ? 'vertical' : 'horizontal';
          this.handleDevToolsDetected();
        }
      } else {
        devtools.open = false;
  ...
  handleDevToolsDetected() {
    ...
    console.log('%cThis is a browser feature intended for developers. Unauthorized access or manipulation of this website is prohibited.', 'color: red; font-size: 16px;');
